...
def generate_wf():
    ...
    validate.add_profiles(Namespace.PEGASUS, key='glite.arguments', value='--cpus-per-task=14 --gres=cs:cerebras:1 --qos=low')
    ...
    compile.add_profiles(Namespace.PEGASUS, key='glite.arguments', value='--cpus-per-task=14 --gres=cs:cerebras:1 --qos=low')
    ...
    train.add_profiles(Namespace.PEGASUS, key='glite.arguments', value='--cpus-per-task=14 --gres=cs:cerebras:1 --qos=low')
...
#    neocortex.add_env("PEGASUS_HOME", "/ocean/projects/cis240026p/vahi/software/install/pegasus/default")
    ...
    modelzoo_config_params = File("modelzoo/modelzoo/fc_mnist/pytorch/configs/params.yaml")
    modelzoo_raw = File("modelzoo-raw.tgz")
    rc.add_replica('local', modelzoo_config_params.lfn, "{}/input/params.yaml".format(BASE_DIR))
    ...
    for file in ["train-images-idx3-ubyte.gz", "train-labels-idx1-ubyte.gz", "t10k-images-idx3-ubyte.gz", "t10k-labels-idx1-ubyte.gz"]:
        ...
        rc.add_replica('nonlocal', train_file.lfn, "http://yann.lecun.com/exdb/mnist/{}".format(file))
        rc.add_replica('nonlocal', train_file.lfn, "https://ossci-datasets.s3.amazonaws.com/mnist/{}".format(file))
    ...
    training_job.add_args('--mode train --params configs/params.yaml --model_dir model --cs_ip $CS_IP_ADDR')
